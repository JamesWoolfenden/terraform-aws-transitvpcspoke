...
def lambda_handler(event, context):
  ...
  log.info('Getting config file %s/%s%s',bucket_region, bucket_prefix, config_file)
  config=ast.literal_eval(s3.get_object(Bucket=bucket_region,Key=bucket_prefix+config_file)['Body'].read())
  ...
  for vgw in vgws['VpnGateways']:
    ...
    if not spoke_vgw and vpn_existing:
      ...
      log.info('But we are not performing deletes yet')
